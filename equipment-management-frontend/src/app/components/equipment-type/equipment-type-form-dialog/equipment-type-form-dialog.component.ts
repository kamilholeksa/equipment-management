...
export class EquipmentTypeFormDialogComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { type: EquipmentTypeModel },
  ) {
  ...
  onSubmit() {
    if (this.typeForm.valid) {
      ...
      observable.subscribe({
        next: () => {
          this.notificationService.showSuccess('Typ został zapisany');
  ...
  private populateForm(type: EquipmentTypeModel) {
